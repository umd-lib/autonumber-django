...
@pytest.mark.django_db
def test_should_create_auto_number(authorized_client, name_one, collecting_area_one):
  count_before = AutoNumber.objects.count()

  url = reverse('autonumber_create')
  form_data = {'entry_date': '2025-11-10', 'name': name_one, 'collecting_area': collecting_area_one.pk}
...
@pytest.mark.django_db
def test_should_update_auto_number(authorized_client, auto_number_one, collecting_area_two):
  update_data = {'entry_date': '2025-11-11', 'collecting_area': collecting_area_two.pk}
  ...
  assert auto_number_one.collecting_area == collecting_area_two
...
@pytest.mark.django_db
def test_autonumber_list_sorting(authorized_client, name_one, collecting_area_one):
  # Create two specific objects to test sorting
  old_obj = AutoNumber.objects.create(entry_date=date(2020, 1, 1), name=name_one, collecting_area=collecting_area_one)
  new_obj = AutoNumber.objects.create(entry_date=date(2026, 1, 1), name=name_one, collecting_area=collecting_area_one)
