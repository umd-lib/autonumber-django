...
@pytest.mark.django_db
def test_collection_area_views_not_authorized(authenticated_client, collection_area_one):
  response = authenticated_client.get(reverse('collection_area_list'))
  assert response.status_code == 200
  assert 'Not Authorized' in response.text

  response = authenticated_client.get(reverse('collection_area_create'))
  assert response.status_code == 200
  assert 'Not Authorized' in response.text

  response = authenticated_client.get(reverse('collection_area_detail', kwargs={'pk': collection_area_one.pk}))
  assert response.status_code == 200
  assert 'Not Authorized' in response.text

  response = authenticated_client.get(reverse('collection_area_update', kwargs={'pk': collection_area_one.pk}))
  assert response.status_code == 200
  assert 'Not Authorized' in response.text

  response = authenticated_client.get(reverse('collection_area_delete', kwargs={'pk': collection_area_one.pk}))
  assert response.status_code == 200
  assert 'Not Authorized' in response.text
